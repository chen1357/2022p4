...
parser MyParser(packet_in packet,
    ...
    state parse_ethernet {
        ...
        transition select(hdr.ethernet.etherType) {//根据etherType的值（协议类型）选择下一个状态，直到转移到accept
            TYPE_IPV4: parse_ipv4;	//如果是0x0800即TYPE_IPV4,则转移到parse_ipv4状态（解析ip包头）
            TYPE_ARP : parse_arp;	//如果0x0806即TYPE_ARP,则转移到parse_arp状态（解析ip包头）
            default  : accept;		//默认接受，进入下一步处理
        }
    }
    ...
    state parse_arp_ipv4 {
        packet.extract(hdr.arp_ipv4);		//提取arp包头
...
control MyIngress(inout headers hdr,
    ...
    action ipv4_forward(macAddr_t dstAddr, egressSpec_t port) {// 定义转发的动作
    ...
    table ipv4_lpm {
        key = {//流表匹配域关键字
            hdr.ipv4.dstAddr: lpm;//匹配模式是数据包头的ip目的地址（lpm是最长前缀匹配，exact是精准匹配，ternary是三元匹配）
        }
        actions = {//流表动作集合
            ipv4_forward;	//转发数据，自定义的转发动作
            drop;		//丢弃动作
            NoAction;		//无动作
        }
    ...
    action send_arp_reply(macAddr_t mac_da, ip4Addr_t dst_ipv4) {//arp回复
        ...
        hdr.arp_ipv4.sha     = mac_da;
        hdr.arp_ipv4.spa     = dst_ipv4;
    ...
    table forward{
        key = {
            hdr.arp_ipv4.tpa : lpm;	//lpm为最长前缀匹配
        }
        actions = {
            send_arp_reply;
            drop;
